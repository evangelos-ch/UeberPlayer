...
// peppy fix https://github.com/noatpad/UeberPlayer/issues/18
// Set this value higher if you're mostly listening to longer music / mixes
...
const options = {
    ...
    /* Cache setting! */
    cacheMaxDays: 1, // -> 15 (default) | <number>
...
const miniWrapperPos = ({ horizontal }) => {
    switch (horizontal) {
        ...
        default:
            return horizontal.startsWith("-") ? `text-align: right;` : `text-align: left;`;
    }
};
...
const checkForUpdate = async (dispatch) => {
    let resp = await fetch("https://raw.githubusercontent.com/evangelos-ch/UeberPlayer/master/widget.json");
    if (!resp.ok) {
...
const updateSongData = (output, error, previousState) => {
    ...
    let [playing, app, track, artist, album, artworkURL, artworkFilename, duration, elapsed, appleMusicError] = output
        .trim()
        .split(" @@ ");
    ...
    } else if (track !== previousState.song.track || album !== previousState.song.album) {
        // Song change
        return {
            ...previousState,
            app,
        ...
        <UpdateText style={{ color }}>
            {updatePending ? (
                <span style={{ textDecoration: "none", fontStyle: "italic" }}>
                    <b>Updating!</b> You might need to restart Ubersicht when finished... ({Math.floor(_updateTimer)})
                </span>
            ) : (
    ...
    return (
        <SmallPlayer>
            ...
            <Information className="small">
                ...
                {updateAvailable && (
                    <UpdateNotif dispatch={dispatch} updatePending={updatePending} color={secondaryColor} />
                )}
                <Progress
                    progressColor={secondaryColor}
                    emptyColor={tercaryColor}
        ...
        return (
            <Wrapper show={showWidget} horizontal={horizontalPosition} vertical={verticalPosition}>
                <ErrorWrapper>
                    ...
                    <p>
                        Unfortunately, the widget is unable to access songs being streamed online through Apple Music.
                    </p>
